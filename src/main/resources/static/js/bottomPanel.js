    ...
    this.getRoutineData = function (data,data1) {
        ...
        if(data1 != null){
            for(var i=0;i<data1.length;i++){
                if(num > data1[i].dataArrivalLitter.length){
                    num = data1[i].dataArrivalLitter.length;
                }
            }
        }
        for(var j=0;j<num;j++){
            ...
            if(data1 != null){
                routineData += '<div class="swiper-slide timeHeight">' +
                    '<div class="slide-time">'+data1[0].dataArrivalLitter[data1[0].dataArrivalLitter.length-(j+1)].time+'</div></div>'
                routineData += '<div class="slide-des dataHeight"><span class="color '+
                    this.getState(data1[0].dataArrivalLitter[data1[0].dataArrivalLitter.length-(j+1)].centerDataArrival[data1[0].dataArrivalLitter[data1[0].dataArrivalLitter.length-(j+1)].centerDataArrival.length-1].timeliessCode)+'"></span></div>'
            }
            routineData +='</div>'
        }
        return routineData;
